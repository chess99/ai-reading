...
test('home page leads with the reader promise and elevates topic reading', () => {
  ...
  assert.ok(topicIndex > introIndex, 'Topic reading should follow the positioning intro.');
  assert.ok(topicIndex < newBooksIndex, 'Topic reading should appear before the latest-book inventory.');
});
